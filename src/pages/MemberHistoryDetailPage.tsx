...
export default function MemberHistoryDetailPage() {
    ...
    const load = async () => {
      try {
        ...
        if (history.modifieParId) {
          ...
          setUserName(u?.username || `User #${history.modifieParId}`);
        } else {
          setUserName("Système");
        }
    ...
    return (
      <div style={{ maxWidth: 600, margin: "40px auto", padding: 16 }}>
        ...
        <div style={{ textAlign: "center", marginTop: 16 }}>
          <button
            onClick={() => (window.location.href = "/member-histories")}
            style={{
              padding: "8px 16px",
              background: "#f3f4f6",
  ...
  const rows = [
    ...
    {
      label: "Ancien statut",
      value: data.ancienStatut
        ? StatutMembreLabels[data.ancienStatut]
        : "Création",
    },
    {
      label: "Nouveau statut",
      value: StatutMembreLabels[data.nouveauStatut],
    },
    { label: "Motif", value: data.motif || "—" },
    {
      label: "Date",
      value: new Date(data.dateChangement!).toLocaleString(),
    },
