...
import { getBureauByAssociation, getBureauActifByAssociation, deleteBureau, cloturerBureau } from "../api/bureauService";
...
export default function BureauListPage() {
  ...
  const [bureaux, setBureaux]           = useState<BureauDto[]>([]);
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      toast.success("🗑️ Poste supprimé");
  ...
  const handleCloturer = async (id: number) => {
    try {
      await cloturerBureau(id);
      toast.success("✅ Poste clôturé");
      const fn = filtreActif ? getBureauActifByAssociation : getBureauByAssociation;
      fn(Number(selectedAssoc)).then(setBureaux);
    } catch {
      toast.error("❌ Erreur clôture");
    }
  };

  // Organigramme — grouper par poste
  const grouped = bureaux.reduce((acc, b) => {
    const key = b.poste;
    if (!acc[key]) acc[key] = [];
    acc[key].push(b);
    return acc;
  }, {} as Record<string, BureauDto[]>);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f8fafc", fontFamily: "'Inter','Segoe UI',system-ui,sans-serif", padding: "28px 24px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        ...
        {isAdminOrSuperAdmin && (
          <button
            ...
            style={{ background: "linear-gradient(135deg,#1d4ed8,#2563eb)", color: "#fff", border: "none", padding: "10px 20px", borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: "pointer", boxShadow: "0 2px 8px rgba(29,78,216,0.3)" }}
          >
            + Nouveau poste
          </button>
        )}
      ...
      <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e5e7eb", padding: "14px 18px", marginBottom: 20, display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        <select style={inputStyle} value={selectedAssoc} onChange={e => setSelectedAssoc(Number(e.target.value))}>
          {associations.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
        </select>

        <div style={{ display: "flex", gap: 8 }}>
          {[true, false].map(v => (
            <button key={String(v)} onClick={() => setFiltreActif(v)}
      ...
      {loading ? (
                ...
                <div style={{ padding: "20px" }}>
                  ...
                  {isAdminOrSuperAdmin && (
                    <div style={{ display: "flex", gap: 8, marginTop: 14, paddingTop: 14, borderTop: "1px solid #f1f5f9" }}>
                      <button onClick={() => navigate(`/bureaux/${b.id}/edit`)}
                      ...
                      {b.actif && (
                        <button onClick={() => handleCloturer(b.id!)}
        ...
        <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e5e7eb", padding: "32px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 40 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "linear-gradient(135deg,#1d4ed8,#2563eb)", color: "#fff", padding: "12px 28px", borderRadius: 12, fontSize: 16, fontWeight: 700, boxShadow: "0 4px 16px rgba(29,78,216,0.3)" }}>
              🏛️ {associations.find(a => a.id === Number(selectedAssoc))?.name || "Association"}
            </div>
            {/* Ligne vers le bas */}
                      ...
                      <div key={b.id} style={{ background: "#fff", border: `1px solid ${st.border}`, borderRadius: 12, padding: "16px 20px", textAlign: "center", minWidth: 160, boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
                        ...
                        {isAdminOrSuperAdmin && (
                          <div style={{ display: "flex", gap: 6, marginTop: 10, justifyContent: "center" }}>
                            <button onClick={() => navigate(`/bureaux/${b.id}/edit`)}
                              style={{ padding: "4px 10px", borderRadius: 6, background: "#f0fdf4", color: "#16a34a", border: "none", fontSize: 12, cursor: "pointer", fontWeight: 600 }}>✏️</button>
                            {b.actif && (
                              <button onClick={() => handleCloturer(b.id!)}
                                style={{ padding: "4px 10px", borderRadius: 6, background: "#fffbeb", color: "#d97706", border: "none", fontSize: 12, cursor: "pointer", fontWeight: 600 }}>🔒</button>
                            )}
                            <button onClick={() => setConfirmId(b.id!)}
                              style={{ padding: "4px 10px", borderRadius: 6, background: "#fef2f2", color: "#dc2626", border: "none", fontSize: 12, cursor: "pointer", fontWeight: 600 }}>🗑️</button>
                          </div>
                        )}
                      </div>
                    ))}
